...
in
{
  ...
  systemd.services.get-obs-thingy-sourcething-id = {
    ...
    serviceConfig.ExecStart = "${pkgs.wireplumber}/bin/wpctl status | grep -A 5 Sources | grep obs | cut -c 1-20 | grep -o '[0-9]''\\+' >> /home/j/Scripts/obs-thingy-sourcething-id.txt";
